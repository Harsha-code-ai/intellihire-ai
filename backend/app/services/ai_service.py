...
FIX NOTES
---------
* openai>=1.0 + httpx mismatch caused 'proxies' TypeError.
  Fixed by passing timeout via httpx.Client explicitly.
* All public functions have fallbacks — never crash.
"""
...
def _get_client():
    ...
    if _client is None:
        try:
            ...
            # ✅ FIX: pass timeout via httpx.Client to avoid 'proxies' TypeError
            _client = OpenAI(
...
def _fallback_resume_analysis(text: str) -> dict:
    ...
    keywords = ["python", "java", "javascript", "react", "node", "sql",
    ...
    return {
        ...
        "resume_score":     float(min(100, len(skills) * 10 + 20)),
        "score_breakdown": {
